import { Course, Module, ViewWithSlides, LocalSlide, Assessment, ContentInfo, DatasetInfo, ViewInfo, ViewType } from "@/types/index";
...
// Create assessments in bulk for a view
export const createAssessments = async (viewId: number, assessmentData: AssessmentData[]): Promise<ViewType> => {
	try {
		const response = await admin_api.post<ViewType>(`/views/${viewId}/assessments`, {assessments: assessmentData});
		return response.data;
	} catch (error) {
...
// Remove slides from a view
export const removeSlidesFromView = async (viewId: number, slideIds: number[]): Promise<ViewType> => {
	try {
		const response = await admin_api.delete<ViewType>(`/views/${viewId}/slides`, { data: { slides: slideIds } });
		return response.data;
	} catch (error) {
		console.error("Error removing slides from view", error);
		throw error;
	}
};
