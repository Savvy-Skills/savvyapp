...
export default function AudioEncodingLesson({ content }: { content?: ContentInfo }) {
	...
	// Delayed initialization for default audio
	useEffect(() => {
		if (!audioInitialized && !audioUrl && !audioFile) {
			const timer = setTimeout(() => {
				setAudioUrl(DEFAULT_LOCAL_SOUNDS[0].url);
				setAudioInitialized(true);
			}, 300); // 300ms delay before loading the default audio
			
			return () => clearTimeout(timer);
		}
	}, [audioInitialized, audioUrl, audioFile]);
	...
	const handleConfigChange = useCallback((e: React.ChangeEvent<HTMLSelectElement>) => {
	...
	// Throttle audio selection to prevent rapid changes
	const handleAudioUrlSelect = (url: string | null) => {
		...
		// Use setTimeout to give the component time to clean up previous audio
		setTimeout(() => {
			setAudioUrl(url);
		}, 100);
	};
	...
	useEffect(() => {
		if (wavesurfer) {
			...
			// Add cleanup function for the record plugin
			return () => {
				if (recordPlugin) {
					if (isRecording) {
	...
	useEffect(() => {
		return () => {
			...
			if (recordResult && recordResult.startsWith('blob:')) {
				URL.revokeObjectURL(recordResult);
			}
		};
	}, [recordingTimer, recordResult]);

	return (
		<>
			{step == "waveform"  && content?.mode == "simple" && (
				<div>
			...
			>
				...
				<div className="audio-source-selector">
					<div className="input-section">
						<p className="option-title">{content?.mode === "advanced" ? "Option 1: Sample Sounds" : "Sample Sounds"}</p>
					...
					{content?.mode === "advanced" && (
						<div className="input-section">
							...
							<div className="center-buttons">
								...
								{step != "waveform" && (
									<div ref={waveformRef} className="waveform-container" style={{ position: "absolute", top: 0, left: 0, zIndex: -1 }} />
								)}
							</div>
						</div>
					)}
			...
			{hasAudioSource && step == "spectrogram" && (
				<StepCard
					stepNumber={2}
